...
void main() {
  runApp(const SimpleTaskFix());
}

class SimpleTaskFix extends StatelessWidget {
  const SimpleTaskFix({super.key});
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      ...
      home: const SimpleTaskPage(),
    );
  }
}

class SimpleTaskPage extends StatefulWidget {
  const SimpleTaskPage({super.key});

  @override
  State<SimpleTaskPage> createState() => _SimpleTaskPageState();
}

class _SimpleTaskPageState extends State<SimpleTaskPage> {
    ...
    for (int week = 0; week < 6; week++) {
      ...
      for (int day = 0; day < 7; day++) {
        ...
        if (index < firstWeekday) {
          days.add(const SizedBox(height: 60));
